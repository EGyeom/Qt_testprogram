/****************************************************************************
** Meta object code from reading C++ file 'uarttest.h'
**
** Created: Thu Apr 16 18:57:16 2020
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.4)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../uarttest.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'uarttest.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.4. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_uartTest[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: signature, parameters, type, tag, flags
      19,   10,    9,    9, 0x05,

 // slots: signature, parameters, type, tag, flags
      34,    9,    9,    9, 0x08,
      56,    9,    9,    9, 0x08,
      77,    9,    9,    9, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_uartTest[] = {
    "uartTest\0\0hasFocus\0focussed(bool)\0"
    "on_Input_bt_clicked()\0on_exit_bt_clicked()\0"
    "buttonClicked(int)\0"
};

void uartTest::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        uartTest *_t = static_cast<uartTest *>(_o);
        switch (_id) {
        case 0: _t->focussed((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 1: _t->on_Input_bt_clicked(); break;
        case 2: _t->on_exit_bt_clicked(); break;
        case 3: _t->buttonClicked((*reinterpret_cast< int(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData uartTest::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject uartTest::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_uartTest,
      qt_meta_data_uartTest, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &uartTest::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *uartTest::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *uartTest::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_uartTest))
        return static_cast<void*>(const_cast< uartTest*>(this));
    return QWidget::qt_metacast(_clname);
}

int uartTest::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    }
    return _id;
}

// SIGNAL 0
void uartTest::focussed(bool _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}
QT_END_MOC_NAMESPACE
